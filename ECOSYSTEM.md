...
> **Audio platform — demucs/basic-pitch/yt-dlp pipeline at nuit.one.**
...
Nuit One is MADFAM's audio-processing platform: stem separation (demucs), pitch detection (basic-pitch), YouTube download (yt-dlp) served behind a SvelteKit-style web + Node API. Primary use: music education + remix tooling. Domain: `nuit.one`.
...
### Day-to-day for nuit-one-api

The commands below default to `nuit-one-api` — the primary service name for
...
enclii ps --wide
enclii ps nuit-one-api --env production

# Logs (tail, filter, history)
enclii logs nuit-one-api -f                          # live tail
enclii logs nuit-one-api --since 1h --level error    # last hour, errors only
enclii logs nuit-one-api --env staging -f
...
# Rollback
enclii rollback nuit-one-api                         # previous release
enclii rollback nuit-one-api --to-revision 5

# Releases + history
enclii releases nuit-one-api                          # list builds
enclii releases nuit-one-api --latest --output json

# Secrets (routed through Lockbox → Vault → ESO → K8s)
enclii secrets list nuit-one-api
enclii secrets set MY_KEY=value --service nuit-one-api --secret
enclii secrets rm MY_KEY --service nuit-one-api

# Domains, tunnel routes, DNS
enclii domains list nuit-one-api
enclii domains add nuit-one-api my.example.com       # auto-provisions tunnel route + DNS
...
# Routing (ingress + TLS)
enclii junctions list nuit-one-api
